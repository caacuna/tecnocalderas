<?php
	if(!empty($_POST)) {
		...
		$query = "INSERT INTO caldera(id_comuna, id_marca, id_inst, id_alimentacion, ano, pasos, latitud, longitud, id_usuario, capacidad, tipo_espalda, tipo_tubular, orientacion)
			VALUES	($id_comuna, $id_marca, $id_inst, $id_alimentacion, $ano, $pasos, '$latitud', '$longitud', $id_usuario, $capacidad, $tipo_espalda, $tipo_tubular, '$orientacion');";
		$result = pg_query($query);
		if($result) {
			set_alert('Caldera agregada correctamente.', 'success');
			header("Location: " . mod_link('calderas'));
			die();
		}
	}

	$titulo = 'Agregar Caldera';
...
<form method="post" action="<?php echo mod_link('calderas', 'agregar'); ?>" class="needs-validation" novalidate>
	<div class="form-row">
		<div class="col-md-6 mb-3">
			<label for="id_comuna">Comuna</label>
			<select class="form-control" name="id_comuna" id="id_comuna" required>
				<option value="">Seleccione Comuna</option>
				<?php foreach($comunas as $opt) printf('<option value="%s"%s>%s</option>', $opt['id_comuna'], sopt_selected('id_comuna', $opt['id_comuna']), $opt['nombre_comuna']); ?>
			</select>
	...
	<div class="form-row">
		<div class="col-md-6 mb-3">
			...
			<select class="form-control" name="id_marca" id="id_marca" required>
				...
				<?php foreach($marcas as $opt) printf('<option value="%s"%s>%s</option>', $opt['id_marca'], sopt_selected('id_marca', $opt['id_marca']), $opt['nom_marca']); ?>
			</select>
			<div class="invalid-feedback">
				Seleccione Marca
			</div>
		...
		<div class="col-md-6 mb-3">
			...
			<select class="form-control" name="id_alimentacion" id="id_alimentacion" required>
				...
				<?php foreach($alimentaciones as $opt) printf('<option value="%s"%s>%s</option>', $opt['id_alimentacion'], sopt_selected('id_alimentacion', $opt['id_alimentacion']), $opt['nom_alim']); ?>
			</select>
			<div class="invalid-feedback">
	...
	<div class="form-row">
		<div class="col-md-4 mb-3">
			...
			<input type="text" name="ano" class="form-control" id="ano" placeholder="Año" value="<?php ival('ano'); ?>" pattern="\d{4,4}" required>
			<div class="invalid-feedback">
		...
		<div class="col-md-4 mb-3">
			...
			<input type="text" name="pasos" class="form-control" id="pasos" placeholder="Nro pasos" value="<?php ival('pasos'); ?>" pattern="\d*">
			<div class="invalid-feedback">
		...
		<div class="col-md-4 mb-3">
			...
			<div class="input-group">
				<input type="text" name="capacidad" class="form-control" id="capacidad" placeholder="Capacidad" value="<?php ival('capacidad'); ?>" pattern="\d*" required>
	...
	<div class="form-row">
		<div class="col-md-6 mb-3">
			...
			<input type="text" name="latitud" class="form-control" id="latitud" placeholder="Latitud" value="<?php ival('latitud'); ?>" required>
			<div class="invalid-feedback">
				Ingrese Latitud
			</div>
		</div>
		<div class="col-md-6 mb-3">
			...
			<input type="text" name="longitud" class="form-control" id="longitud" placeholder="Longitud" value="<?php ival('longitud'); ?>" required>
			<div class="invalid-feedback">
				Ingrese Longitud
			</div>
		</div>
	...
	<div class="form-row">
		<div class="col-md-6 mb-3">
			...
			<select class="form-control" name="orientacion" id="orientacion" required>
				...
				<?php foreach($orientaciones as $opt) printf('<option value="%s"%s>%s</option>', $opt['orientacion'], sopt_selected('orientacion', $opt['orientacion']), $opt['nombre_orientacion']); ?>
			</select>
			<div class="invalid-feedback">
		...
		<div class="col-md-6 mb-3">
			...
			<select class="form-control" name="id_usuario" id="id_usuario" required>
				...
				<?php foreach($tecnicos as $opt) printf('<option value="%s"%s>%s</option>', $opt['id_usuario'], sopt_selected('id_usuario', $opt['id_usuario']), $opt['nombre']); ?>
			</select>
			<div class="invalid-feedback">
	...
	<div class="form-row">
		<div class="col-md-6 mb-3 d-none" id="tipo_espalda-wrap">
			...
			<select class="form-control" name="tipo_espalda" id="tipo_espalda">
				...
				<?php foreach($tipos_espalda as $opt) printf('<option value="%s"%s>%s</option>', $opt['tipo_espalda'], sopt_selected('tipo_espalda', $opt['tipo_espalda']), $opt['nombre_tipo_espalda']); ?>
			</select>
			<div class="invalid-feedback">
		...
		<div class="col-md-6 mb-3 d-none" id="tipo_tubular-wrap">
			...
			<select class="form-control" name="tipo_tubular" id="tipo_tubular">
				...
				<?php foreach($tipos_tubular as $opt) printf('<option value="%s"%s>%s</option>', $opt['tipo_tubular'], sopt_selected('tipo_tubular', $opt['tipo_tubular']), $opt['nombre_tipo_tubular']); ?>
			</select>
			<div class="invalid-feedback">
